    ...
    <div className="animate-fade-up">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Link href="/dashboard/qr-codes/new" className="btn-primary btn-mint no-underline !text-[13px]">+ New QR</Link>
        </div>
      </div>
          ...
          <div className="flex flex-wrap justify-center gap-3 mt-2">
            {devices.map((d, i) => (
              <span key={i} className="text-[11px] font-bold text-brand-text-mid flex items-center gap-1">
                <span className="w-2 h-2 rounded-sm" style={{ background: [COLORS.mint, COLORS.blue, COLORS.orange][i % 3] }} />{d.device} ({d.count})
              </span>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <div className="card p-5">
          ...
          ) : <div className="text-center py-10 text-brand-text-dim text-sm">No data yet</div>}
        </div>
        <div className="card p-5">
