    ...
    <div className="min-h-screen flex items-center justify-center bg-brand-bg px-4">
      <div className="w-full max-w-[400px]">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-brand-text-dim mt-1">Log in to your RedSprout QR dashboard</p>
        </div>
        <div className="card p-6">
          <button onClick={() => signIn('google', { callbackUrl: '/dashboard' })} className="w-full flex items-center justify-center gap-3 py-3 rounded-xl border border-brand-border bg-white text-sm font-bold text-brand-text cursor-pointer transition-all hover:border-brand-border-focus hover:bg-brand-card-alt mb-4">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-4"><label className="label">Password</label><input className="input-field" type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" required /></div>
            <button type="submit" className="btn-primary w-full !py-3.5" disabled={loading}>{loading ? 'Logging in...' : 'Log In'}</button>
          </form>
        </div>
        <p className="text-center text-sm text-brand-text-dim mt-4">Don&apos;t have an account? <Link href="/auth/register" className="text-brand-mint-dark font-bold no-underline">Sign up free →</Link></p>
      </div>
    </div>
  );
}
