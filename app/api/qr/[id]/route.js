    ...
    const qrCode = await prisma.qRCode.findFirst({
      ...
      include: {
        ...
        scans: {
          ...
          take: 10,
          select: { id: true, country: true, city: true, deviceType: true, browser: true, os: true, scannedAt: true },
        },
      },
    });
...
export async function PUT(request, { params }) {
  try {
    const session = await getSession();
    if (!session?.user?.id) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    const existing = await prisma.qRCode.findFirst({ where: { id: params.id, userId: session.user.id } });
    ...
    if (existing.type === 'DYNAMIC' && body.destinationUrl) updates.destinationUrl = body.destinationUrl;
    if (existing.type === 'STATIC' && body.staticData) {
      return NextResponse.json({ error: 'Static QR codes cannot be modified.' }, { status: 400 });
    }
    ...
    const qrCode = await prisma.qRCode.update({ where: { id: params.id }, data: updates });
    return NextResponse.json(qrCode);
  } catch (error) {
